...
class Responder(gym.Env):
    ...
    def reset(self,
        ...
        obs = {"instance": flatten(self.instance[:, 1:], dtype = self._FLOAT),
               "coalition": coalition,
               "proposal": proposal,
               "response": one_hot(proposer, self.n_vehicles, self._BIN),
               "time": self._INT(time),
               "proposer": proposer,
               "agent": self.order.pop(),
               "role": self._INT(role)}
    ...
    def step(self, action: np.ndarray):
        ...
        if self.obs["role"] == 1:
            response = self._BIN(action[_mask]) * one_hot(self.obs["agent"],
                                                          self.n_vehicles,
                                                          self._BIN)
            self.obs["response"] = np.maximum(response, self.obs["response"])
        ...
        if self.obs["role"] == 2:
            ...
            if self.obs["agent"] == 0:
                ...
                coalition, proposal = propose(self.obs,
                                              self.n_vehicles,
                                              self.agent_path,
                                              self.coalitions,
                                              self.gains,
    ...
    def reward(self):
        ...
        reward = coalition[i] * (proposal[i] - (1 / n) + 1e-3) * response[i] * value
